...
const initializeDB = () => {
  const authorDB_sql = 'CREATE TABLE IF NOT EXISTS author( author_id int NOT NULL AUTO_INCREMENT, email varchar(255) NOT NULL UNIQUE, PRIMARY KEY(author_id))';
  ...
  const arrays = [affiliationDB_sql, authorDB_sql, paperDB_sql, authoridDB_sql, testbedDB_sql, authorAffiliationDB_sql, authorPapersDB_sql, testbedPaperDB_sql, testbedSavedPaperDB_sql, GENISetup, CloudlabSetup, ChameleonSetup];
